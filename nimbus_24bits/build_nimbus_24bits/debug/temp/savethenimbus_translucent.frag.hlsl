...
SamplerComparisonState _shadowMap_sampler : register(s3);
uniform float3 eye;
...
{
    float3 _193 = float3(uv + ((-1.0f).xx / smSize), compare);
    float result = shadowMap_1.SampleCmp(_shadowMap_1_sampler, _193.xy, _193.z);
    float3 _202 = float3(uv + (float2(-1.0f, 0.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _202.xy, _202.z);
    float3 _213 = float3(uv + (float2(-1.0f, 1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _213.xy, _213.z);
    float3 _224 = float3(uv + (float2(0.0f, -1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _224.xy, _224.z);
    float3 _232 = float3(uv, compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _232.xy, _232.z);
    float3 _243 = float3(uv + (float2(0.0f, 1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _243.xy, _243.z);
    float3 _254 = float3(uv + (float2(1.0f, -1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _254.xy, _254.z);
    float3 _265 = float3(uv + (float2(1.0f, 0.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _265.xy, _265.z);
    float3 _276 = float3(uv + (1.0f.xx / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _276.xy, _276.z);
    return result / 9.0f;
}
...
float shadowTestCascade(Texture2D<float4> shadowMap_1, SamplerComparisonState _shadowMap_1_sampler, float3 eye_1, float3 p, float shadowsBias_1)
{
    ...
    int param;
    int param_1;
    float4x4 _372 = getCascadeMat(d, param, param_1);
    int casi = param;
    int casIndex = param_1;
    float4x4 LWVP = _372;
    float4 lPos = mul(float4(p, 1.0f), LWVP);
    float3 _387 = lPos.xyz / lPos.w.xxx;
    lPos = float4(_387.x, _387.y, _387.z, lPos.w);
    ...
    if ((splitDist <= 0.1500000059604644775390625f) && (casi != 3))
    {
        ...
        float3 _491 = lPos2.xyz / lPos2.w.xxx;
        lPos2 = float4(_491.x, _491.y, _491.z, lPos2.w);
...
void frag_main()
{
    ...
    float3 _665 = pow(ImageTexture_010_texread_store.xyz, 2.2000000476837158203125f.xxx);
    ImageTexture_010_texread_store = float4(_665.x, _665.y, _665.z, ImageTexture_010_texread_store.w);

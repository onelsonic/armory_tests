...
void frag_main()
{
    ...
    {
        float3 param = ld;
        float _94 = lpToDepth(param, lightProj);
        float compare = _94 - (pointBias * 1.5f);
        l.y = -l.y;
        float4 _128 = float4((-l) + ((n * pointBias) * 20.0f), compare);
        visibility = shadowMapPoint[0].SampleCmp(_shadowMapPoint_sampler[0], _128.xyz, _128.w);
    }
    ...
    float3 _163 = pow(fragColor.xyz, 0.4545454680919647216796875f.xxx);
    fragColor = float4(_163.x, _163.y, _163.z, fragColor.w);
}
